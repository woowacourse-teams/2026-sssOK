...
(`frontend/package.json`)를 따로 관리한다. 백엔드 작업 PR은 이슈별 버전 의도를 남기고, 릴리스
준비 PR에서 누적 의도 중 가장 높은 SemVer 단계를 백엔드 버전에 한 번 반영한다. 통합 릴리스와
프론트엔드 버전도 릴리스 준비 PR에서 올린다. CI가 의도·형식·증가·태그 중복을 검사한다. 릴리스 태그
