...
(`frontend/package.json`)를 따로 관리한다. 백엔드 버전은 작업 PR에서 선택한 SemVer 증가 유형에
따라 자동 반영하고, 통합 릴리스·프론트엔드 버전은 `develop → main` PR 전에 사람이 직접 올린다.
작업 PR과 `main`·`deploy` 대상 PR의 CI가 형식·증가·태그 중복을 검사한다. 릴리스 태그
