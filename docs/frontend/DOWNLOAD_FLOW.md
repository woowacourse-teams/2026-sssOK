...
`navigator.share` 는 **시트를 띄우는 데까지만** 책임진다. 사용자가 시트에서 무엇을 눌렀는지는
프론트가 알 수 없어, 아무것도 저장되지 않아도 `share()` 는 그냥 resolve 한다. 실제로 사진첩에
넣는 것은 시트 아래쪽 액션 목록의 **"이미지 저장"** 이고, 위쪽 아이콘 줄의 사진 관련 앱은
*그 앱으로 보내기*(대개 클라우드 업로드)라 카메라롤에는 남지 않는다.

버튼에 "사진첩에 저장" 이라고만 써 두면 시트에서 한 번 더 골라야 하는 줄 모르고 눈에 먼저 띄는
아이콘을 누르게 된다. 그래서 시트를 고르는 자리에서 어디를 눌러야 하는지 미리 말해준다
(`DownloadModeSheet` 의 `Notice`).
