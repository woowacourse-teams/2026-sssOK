...
| 4   | **실제 상향 회선 속도** | 아무도 안 재봤다. 지금 문서의 1Mbps 는 가정이다. **실기기로, 사람 몰린 실제 행사장에서** 재야 한다 — 여기서 1번이 갈린다                 |
| 5   | **아이폰 실사진 측정**  | 지금 측정은 아이폰 원본이 아니다. 실사진 한 장이면 변환 후 절대 크기와 품질 최종값(`0.80` 잠정)이 같이 확정된다                          |
...
측정에서 끌어온 값이지만, 거기 적어둔 대로 **아이폰 실사진으로는 아직 안 재봤다**(미정 5번).
1Mbps 는 **아무도 재보지 않은 순수한 가정**이다 — 최악값으로는 말이 되지만(행사장은 셀이 붐빈다)
이 숫자 하나가 지금 동시성과 만료 여유를 혼자 정하고 있다(미정 4번).
...
> 절대값이 확정된다(미정 5번). **품질 간 배수 1.62x 만 지금 확정된 값이다.**
...
- **품질은 `0.80` 으로 인코딩한다.** `0.92` 가 아니다. 위 표에서 `0.92` → `0.80` 은
  **크기 38% 감소, PSNR 2.9dB 하락(45.2 → 42.3)** 이다. 두 값 모두 사진에서 원본과
  ...
  (PSNR 은 완벽한 지각 지표가 아니다. 눈으로도 한 번 보고 확정한다 — 아래 미정 5번)
