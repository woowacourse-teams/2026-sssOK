...
- 버전은 사람이 직접 올린다. CI 는 올렸는지 검사만 한다 — Changesets 같은 자동화 도구는
  도입하지 않는다. 이 레포가 npm workspaces 기반 모노레포가 아니라서 비용 대비 실익이 작다.
...
2. `backend/gradle.properties` — 이번 릴리스에 백엔드 변경이 포함되면 올린다
