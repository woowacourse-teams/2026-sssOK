...
| 용도 | API | 응답 |
| --- | --- | --- |
| 커서로 미디어를 점진적으로 조회하는 향후 기능 | `GET /rooms/{roomId}/media` | `items`, `nextCursor`, `hasNext`, `totalCount` |
| 현재 갤러리처럼 전체 미디어가 한 번에 필요한 기능 | `GET /rooms/{roomId}/media/all` | `items` |

두 API 모두 `folderId`, `uploader`를 선택적으로 받아 방 전체 또는 한 폴더의 미디어를
`createdAt DESC, mediaId DESC`로 정렬한다. `uploader`는 `ALL`, `ME`, `OTHERS`를 지원하며
생략하면 `ALL`이다. 페이지 API는 추가로 `cursor`, `size`를 지원한다.

전체 조회는 미디어 수만큼 응답과 서버의 보유 객체가 커진다. 현재 갤러리는 선택된 ID를
클라이언트에서 직접 관리하기 위해 전체 조회를 사용한다. 방별 미디어 수가 커져 다시 점진적 로딩이
필요해질 경우를 대비해 커서 조회 API와 관련 구현은 유지한다.
...
현재 제품 결정은 전체 조회이지만, 대규모 방에서는 커서 조회가 메모리와 응답 크기 측면에서 더 안전하다.
전체 조회 호출이 빈번해지거나 방의 미디어 상한이 커지면 보존한 커서 API로 전환하거나 스트리밍·전용 배치
API로 분리해야 한다.
