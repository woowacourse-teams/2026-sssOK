...
> 과거 기록에 나오는 `deploy.yml`은 당시의 워크플로 이름이다. 현재 운영 배포 워크플로 이름은
> `deploy-prod.yml`이며, 현재 배포 구조는 [배포 가이드](../deployment/DEPLOYMENT.md)를 기준으로 한다.

---

## 2026-09-15 — dev 자동 배포 첫 검증: 디스크 부족과 DB 이름 불일치

`develop`의 백엔드 변경을 dev 서버에 처음 자동 배포하면서, 빌드·이미지 푸시 이후 서버 배포에서
두 가지 운영 설정 문제를 확인했다. 둘 다 애플리케이션 코드가 아니라 dev EC2의 실행 환경 설정이
원인이었다.

| 증상 | 원인 | 해결 |
| --- | --- | --- |
| 러너 로그 생성 중 `No space left on device` | dev EC2 루트 디스크의 여유 공간이 거의 없음 | 사용하지 않는 Docker 이미지를 정리하고, 디스크 사용량을 점검 |
| `FATAL: database "sssok-dev" does not exist` | 실제 DB는 `sssok_dev`인데 dev `.env`가 하이픈 이름을 참조 | `DB_URL`의 DB 이름을 `sssok_dev`로 수정 |

### 1. 디스크 부족으로 러너가 시작 단계에서 실패

**증상**

배포 러너가 진단 로그를 쓰는 과정에서 `No space left on device`로 실패했다. 컨테이너가 없어도
이 오류는 배포 전에 발생할 수 있으므로, 먼저 서버 전체 파일시스템 사용량을 확인해야 한다.
...
dev 서버에는 이전 배포에서 내려받은 미사용 이미지가 남아 있었다. 현재 실행 중인 컨테이너가 없는 것을
확인한 뒤 미사용 이미지를 정리해 여유 공간을 확보했고, 재배포가 진행됐다.

**재발 방지**

- 배포 실패 시 애플리케이션 로그보다 먼저 `df -h`로 루트 디스크 여유 공간을 확인한다.
- 기존의 `docker image prune -f`는 태그가 없는 이미지만 지워, 커밋 SHA 태그가 붙은 이전 배포
  이미지는 계속 남았다. 배포 워크플로는 이제 pull 전과 성공·실패를 포함한 종료 시점에 백엔드 이미지
  저장소를 정리한다.
- 현재 실행 대상(`image.env`)과 롤백 대상(`image.env.prev`)은 정리에서 제외한다. 그보다 오래된
  태그 이미지만 제거하므로 디스크 사용량을 제한하면서 한 단계 롤백은 유지한다.
- 디스크가 작은 인스턴스는 이미지 수와 여유 공간을 주기적으로 점검하고, Docker 이미지 정리만으로
  충분하지 않으면 로그를 정리하거나 볼륨 용량을 확장한다.

### 2. dev 데이터베이스 이름 불일치

**증상**

이미지 빌드·푸시와 컨테이너 기동은 성공했지만, 애플리케이션이 PostgreSQL의
`database "sssok-dev" does not exist` 오류로 기동하지 못했다.

**원인과 해결**

RDS에 생성한 실제 dev 데이터베이스 이름은 `sssok_dev`였는데, 서버 `.env`의 `DB_URL`에는
`sssok-dev`가 설정돼 있었다. DB 이름은 하이픈과 언더스코어를 서로 대체하지 않으므로,
`DB_URL`의 경로를 실제 DB 이름인 `sssok_dev`로 맞춘 뒤 배포에 성공했다.

**재발 방지**

- DB 생성 SQL과 서버 `.env`의 DB 이름을 같은 표기(`sssok_dev`)로 유지한다.
- "DB가 없다"는 오류는 권한 오류와 구분한다. 권한 문제라면 보통 인증·접근 거부 오류가 나타난다.
