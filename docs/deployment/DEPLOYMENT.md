...
릴리스 PR을 만들기 전에 통합 릴리스와 프론트엔드 버전을 올린다. 백엔드 버전은 각 작업 PR에서
선택한 SemVer 증가 유형에 따라 이미 반영된다 — 자세한 기준은 아래에 정리돼 있다.
...
실패하면 통합 릴리스·프론트엔드 기준 파일을 올려 다시 커밋한다. 백엔드 버전이 어긋났다면 누락된
작업 PR의 버전 선택과 자동 반영 결과를 먼저 확인한다.
