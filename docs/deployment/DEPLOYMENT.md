...
승격 PR을 만들기 전에 릴리스 준비 PR에서 통합 릴리스와 필요한 서비스 버전을 올린다. 백엔드
버전은 작업 PR에서 누적한 버전 의도를 집계해 한 번 반영한다 — 자세한 기준은 아래에 정리돼 있다.
...
실패하면 각 기준 파일과 백엔드 버전 의도 집계 결과를 확인해 릴리스 준비 PR을 수정한다.
