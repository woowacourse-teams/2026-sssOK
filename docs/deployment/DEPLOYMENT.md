...
- 이미지 태그는 커밋 SHA를 사용한다. dev 이미지는 `dev-<커밋 SHA>`로, prod 이미지는 `<커밋 SHA>`로
  식별한다. 롤백은 직전 태그로 되돌리는 것으로 끝난다. prod 이미지에는 사람이 읽기 위한
  `backend-vX.Y.Z`·`release-vX.Y.Z` 별칭 태그도 같이 붙지만, 배포·롤백에는 쓰지 않는다
  (별칭은 다음 릴리스에서 다른 이미지로 옮겨간다 — [버전과 릴리스 태그](#버전과-릴리스-태그) 참고).
...
`deploy-prod.yml` 의 `릴리스 태그 생성` 잡이 **운영 헬스체크까지 성공한 뒤에만** 실행된다.
배포가 실패해 롤백되면 그 버전의 태그는 만들어지지 않는다.

- `release-v<통합 릴리스 버전>` — 항상 만든다. 같은 이름이 이미 있으면 버전을 올리지 않은
  재배포로 보고 잡 전체를 건너뛴다.
