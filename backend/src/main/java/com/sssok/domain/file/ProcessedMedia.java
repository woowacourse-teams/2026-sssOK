...
public record ProcessedMedia(StorageKey thumbnailKey, Integer width, Integer height,
    ...
    public static ProcessedMedia none() {
        return new ProcessedMedia(null, null, null, null, null, null);
    }

    // 사진. 재생시간이 없고, 대신 EXIF 에서 촬영 시각과 좌표가 나온다.
    public static ProcessedMedia ofImage(StorageKey thumbnailKey, Integer width, Integer height,
                                         Instant takenAt, GeoPoint location) {
        return new ProcessedMedia(thumbnailKey, width, height, null, takenAt, location);
    }
    ...
    public static ProcessedMedia ofVideo(StorageKey thumbnailKey, Integer width, Integer height,
                                         ...
                                         GeoPoint location) {
        return new ProcessedMedia(thumbnailKey, width, height, durationSeconds, takenAt, location);
    }
}