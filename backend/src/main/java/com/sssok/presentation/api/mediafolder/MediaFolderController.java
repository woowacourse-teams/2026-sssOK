...
public class MediaFolderController {
    ...
    @Operation(
        summary = "폴더에 담기",
        description = "mediaIds로 지정한 미디어를 folderId에 담는다. 이미 속한 다른 폴더는 유지되며, "
            ...
            + "거부한다(404). mediaIds 또는 folderId가 없거나 ID 값이 잘못되면 400이 난다. 빈 배열에는 이벤트를 "
            + "발행하지 않는다. 성공하면 SSE로 "
    ...
    @Operation(
        summary = "폴더에서 꺼내기",
        description = "mediaIds로 지정한 미디어를 folderIds가 가리키는 폴더들에서 꺼낸다. "
            ...
            + "않는 mediaId는 건너뛰고 notFoundMediaIds로 알려준다. mediaIds가 잘못되면 400, folderIds 중 "
