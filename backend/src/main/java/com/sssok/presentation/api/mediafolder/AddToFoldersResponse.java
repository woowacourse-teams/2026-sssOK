...
public record AddToFoldersResponse(
    @Schema(description = "새로 생긴 조합 수") int updatedCount,
    @Schema(description = "이미 담겨 있어 변화가 없던 미디어 수") int alreadyInCount,
    @Schema(description = "존재하지 않아 건너뛴 미디어 ID") List<Long> notFoundMediaIds,
    @Schema(description = "변경 후 대상 폴더의 최신 상태(칩의 개수 표시 갱신용)") FolderSummaryResponse folder
) {
    ...
    public static AddToFoldersResponse from(AddMediaToFoldersResult result) {
        return new AddToFoldersResponse(
            ...
            FolderSummaryResponse.from(result.folder())
        );
    }
}
