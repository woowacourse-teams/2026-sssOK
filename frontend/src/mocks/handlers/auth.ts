...
const readNextUserId = () => {
  try {
    return Number(localStorage.getItem(COUNTER_KEY)) || FIRST_USER_ID;
  } catch {
    return FIRST_USER_ID;
  }
};

const writeNextUserId = (userId: number) => {
  try {
    localStorage.setItem(COUNTER_KEY, String(userId));
  } catch {
    // 저장소를 못 쓰면 이번 세션 안에서만 번호가 이어진다
  }
};

/** 테스트끼리 번호가 이어지지 않도록 되돌린다. */
export const resetAnonymousAuth = () => {
  try {
