 ...
 */
export const trackUpload = (result: UploadResult, durationMs: number) => {
  ...
  if (failedCount > 0) {
    track("Photo Upload Failed", {
      reason: result.failed[0]?.code ?? "REJECTED",
      failed_count: failedCount,
    });
  }
};
...
export const trackUploadError = (fileCount: number) => {
  track("Photo Upload Failed", { reason: "REQUEST_FAILED", failed_count: fileCount });
};
