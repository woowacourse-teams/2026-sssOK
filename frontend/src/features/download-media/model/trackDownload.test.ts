...
describe("trackDownload", () => {
  it("방 20장 중 5장을 골라 받으면 골라 받기로 남긴다", () => {
    trackDownload(
      ...
      { mode: "zip", source: "gallery", selectedCount: 5, roomPhotoCount: 20 },
    );
  ...
  it("방 전체를 골라 받으면 전체 받기로 남긴다", () => {
    trackDownload(
      ...
      { mode: "share", source: "gallery", selectedCount: 20, roomPhotoCount: 20 },
    );
  ...
  it("한 장도 못 받았으면 실패로 남긴다", () => {
    trackDownload(
      ...
      { mode: "individual", source: "gallery", selectedCount: 2, roomPhotoCount: 20 },
    );
  ...
  it("판 전체가 무너지면 사유와 함께 고른 장수만큼 실패로 남긴다", () => {
    trackDownload(
      ...
      { mode: "zip", source: "gallery", selectedCount: 7, roomPhotoCount: 20 },
    );
